...
export async function getProposalListSummaries(): Promise<ProposalListRow[]> {
  const rows = await db
    .select({
      ...
      closedValueMinor: proposals.closedValueMinor,
      closedCurrency: proposals.closedCurrency,
    })
    .from(proposals)
  ...
  return Promise.all(
    rows.map(async (row) => {
      ...
      const valueRaw = row.closedValueMinor != null ? row.closedValueMinor / 100 : row.invoiceTotal;
      const currency = row.closedValueMinor != null ? row.closedCurrency ?? row.currency ?? "USD" : row.currency ?? "USD";
      return {
        id: row.id,
