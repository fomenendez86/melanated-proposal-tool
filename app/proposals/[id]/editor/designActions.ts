...
export async function updateProposalDesign(
  ...
  try {
    db.transaction((transaction) => {
      const payload = { designId: design.id, version: design.version };
      if (existing) {
        transaction
          ...
          .where(
            and(
              eq(proposalSections.id, existing.id),
              eq(proposalSections.proposalId, proposalId)
            )
          )
          .run();
        ...
        transaction
          ...
          .values({ proposalId, sectionType: "documentDesign", sortOrder: -2, payload })
          .run();
      }
