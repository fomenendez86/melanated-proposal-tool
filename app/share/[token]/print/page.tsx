...
export default async function SharedProposalPrintPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return <main className="bg-white"><ProposalRenderer data={record.revision.data} />{signatures.length ? <div data-proposal-page data-proposal-section-type="signatureCertificate" style={{ breakAfter: "auto" }}><SignatureCertificatePage signatures={signatures} /></div> : null}</main>;
}
