...
export default async function SharedProposalPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  if (!record) notFound();
  if (isSharedProposalExpired(record.settings.expiresAt)) {
  ...
  if (record.settings.accessKey) {
    ...
    if (cookieStore.get(shareCookieName(token))?.value !== record.settings.accessKey) return <SharePasswordForm token={token} />;
  }
  await recordShareEvent(record.revision.proposalId, token, "opened");
  const pages = record.revision.data.sections.map((section, index) => <ProposalSectionView key={`${section.type}-${index}`} section={section} />);
  return <main className="min-h-dvh bg-stone-200"><header className="sticky top-0 z-20 flex min-h-16 items-center justify-between border-b border-black/10 bg-white/95 px-4 backdrop-blur sm:px-8"><div><p className="text-xs font-bold uppercase tracking-widest text-emerald-800">Melanated Safaris</p><h1 className="text-sm font-semibold text-stone-900 sm:text-base">Your travel proposal</h1></div><p className="text-xs text-stone-500">Shared {new Date(record.settings.createdAt).toLocaleDateString("en-US", { year:"numeric", month:"short", day:"numeric" })}</p></header><ResponsiveProposalDocument pages={pages} width={record.revision.design.page.widthPx} height={record.revision.design.page.heightPx}/><ShareApproval token={token}/><footer className="py-8 text-center text-xs text-stone-500">This approval applies to the revision shared on {new Date(record.revision.createdAt).toLocaleDateString()}.</footer></main>;
}
