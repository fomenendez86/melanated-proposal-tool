    ...
    <div className="relative box-border h-[1056px] w-[816px] bg-[#f7f5f0] px-[82px] py-12 text-[var(--design-primary,#20252b)]">
      ...
      {lineItems?.length && totals ? (
        <div {...editableRegion("pricingItemsText", "collection")} className="mt-6">
          <p className="text-[11px] font-bold uppercase tracking-[0.15em] text-[var(--design-secondary,#68727d)]">Package Pricing</p>
          <table className="mt-2 w-full border-collapse text-left text-xs">
            <thead><tr className="border-b border-[var(--design-primary,#20252b)]"><th className="py-2 pr-2">Item</th><th className="px-2 py-2 text-right">Qty</th><th className="px-2 py-2 text-right">Unit</th><th className="py-2 pl-2 text-right">Total</th></tr></thead>
            <tbody>
              {lineItems.filter((item) => item.selected).map((item) => (
                <tr key={item.key} className="border-b border-[var(--design-secondary,#68727d)]/20">
                  <td className="py-2 pr-2 font-semibold">{item.description}{item.optional ? <span className="ml-1 font-normal text-[var(--design-secondary,#68727d)]">(optional)</span> : null}</td>
          ...
          <dl className="ml-auto mt-3 w-64 space-y-1 text-xs">
            ...
            <div className="flex justify-between border-t border-[var(--design-primary,#20252b)] pt-1 text-sm font-bold"><dt>Total</dt><dd>{formatMinorMoney(totals.totalMinor, totals.currency)}</dd></div>
          </dl>
        </div>
      ) : <div className="mt-6">
        <p className="text-[11px] font-bold uppercase tracking-[0.15em] text-[var(--design-secondary,#68727d)]">Package Pricing</p>
        <KeyValueList lines={data.packagePricing} className="mt-2" />
      </div>}

      <div {...editableRegion("paymentScheduleText", "multiline")} className="mt-6">
        <p className="text-[11px] font-bold uppercase tracking-[0.15em] text-[var(--design-secondary,#68727d)]">Payment Schedule</p>
        <KeyValueList lines={data.paymentSchedule} className="mt-2" />
      </div>

      <div className="mt-6">
        <p className="text-[11px] font-bold uppercase tracking-[0.15em] text-[var(--design-secondary,#68727d)]">Banking Information</p>
